...
# 制作流程硬节点
- Courseplay Phase 1 先由 A-page JSON 确定性初始化模块化 `outline.md` 外壳,再以 A-page 为单位执行 script block→局部审查→原位填充 outline section→局部审查;禁止整份覆盖 outline,全部 section 与全局派生区通过审查后才停在唯一的 Checkpoint Plan。Confidence: 0.95
