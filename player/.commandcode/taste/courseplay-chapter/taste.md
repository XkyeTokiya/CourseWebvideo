...
- Outline 的页面配方、结构指纹、关系机制、持续元素、可见标题、scene/state 和场景例外是实现硬约束;三源重组只能在这些槽位内设计,screen_guidance 脱离 G/S 清单不等于脱离 outline 结构,不得借“自由设计”丢掉标题或改章结构。Confidence: 0.95
...
- 每个 step 的揭示顺序与口播报到点对齐,关键结论在前提之后出现;元素在口播报到它时落位,末句元素紧随前序级联并在本拍收完,不得把空拍误判为下一 step。实例秒数只作调试参考,不得固化为跨章阈值。Confidence: 0.9
