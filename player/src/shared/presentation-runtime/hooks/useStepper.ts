/* eslint-disable react/set-state-in-effect -- preserved legacy HMR cursor sanitization */
...
export function useStepper(
  chapters: ChapterDef[],
  storageKey = "presentation-cursor-v8",
): StepperState {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowRight" || e.key === " ") {
        if (e.key === " " && document.querySelector("[data-auto-start-gate]")) {
          return;
        }
        e.preventDefault();
        next();
