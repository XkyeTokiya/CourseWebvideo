# TTS Providers（历史独立项目兼容资料）

> 统一 Studio 已迁移到根级 Node provider：`tools/tts-providers/*.mjs`。正式流程见 `references/AUDIO.md` 与根级 `docs/tts-providers.md`。本文件只供旧 `presentation/` 项目参考；新实例禁止复制这些 `.sh`。
