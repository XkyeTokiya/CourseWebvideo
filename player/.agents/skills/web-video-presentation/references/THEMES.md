...
新建 episode 时可直接指定主题；创建器会校验主题目录是否完整：
...
把 `episodes/<id>/project.json` 的 `theme` 字段改成目标主题 id，刷新
播放页完成。章节代码一行没动。`episode:check` 会校验该字段指向的主题
真实存在，不存在的主题会让检查失败。
...
共享运行时 `base.css` 给**性格 token 都准备了合理的默认值**。主题的 `tokens.css`
...
> 完全自由。
>
> 共享运行时还保留 `--stage-bg` / `--stage-text` / `--stage-muted` 三个旧章节
> 兼容别名，分别映射到 `--surface` / `--text` / `--text-mute`。新章节禁止
> 使用这些旧别名；不要在单个主题里重复定义兼容层。
...
```powershell
Copy-Item .agents/skills/web-video-presentation/themes/monochrome-print `
  .agents/skills/web-video-presentation/themes/my-theme -Recurse
```
...
```powershell
pnpm dev
# 使用命令输出的站点 URL，打开 /play/<一个实例的 id>/
```

把新主题放进 `themes/<id>/`（`theme.json` + `tokens.css` 齐全即被
Studio 构建期自动发现），再把该实例 `project.json.theme` 临时指到新
主题，逐章点完验收。检查：
...
主题目录缺少 `theme.json` 或 `tokens.css`、元数据 id 与目录名不一致、实例
引用不存在主题，均不得交付。
...
- **章节 CSS 硬编码 hex 颜色 / 字体名** —— 优先改用已有语义 token；只有
  可跨主题复用的稳定语义缺口，才扩展契约并同步全部主题（注意：**字号 /
  间距 / 时长**硬编码不算反模式，章节按内容自由设计）
- **演示中途切换主题** —— 选一个，一以贯之
