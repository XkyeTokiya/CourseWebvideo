...
- `Scene · Semantic state`：当前仍在哪个场景，以及本步结束时画面形成了什么
  稳定结果；
- `本步场景指令`：若稳定画面改变，说明可见内容或关系如何变化；若只是 narration
  同步 checkpoint，直接写明复用哪个 state、保持哪些内容，不得为填满一行虚构
  “建立/补充/落位/收束”。一个 step 内若有多个内部动作，说明顺序和最终结果。
...
```markdown
（合并：点名与解释服务同一内容单元，没有独立播放控制价值）
（保留两拍并复用 state：需要两个音频 checkpoint，稳定画面不变）
（保留两拍并切换 state：连续链路到达新的可见里程碑，需要同步展示）
```
...
**新 Beat 不等于新 semantic state。** 不要先规定每个 A-page 的 step 数，也不要从
关系机制或槽位数量反推。按以下顺序：
...
- 只有可见信息集合、视觉权重、焦点对象或关系结果发生真实变化时，才创建新 state；
- 若稳定画面与前一步相同，**逐字复用同一 semantic state 名称**，并用“保持……”
  说明 narration 继续由该画面承载；这不构成空 step，也不增加制作义务；
...
三类情况必须特别检查：同一内容单元内的“点名→解释”不因两句口播创建两个
state；薄的设问/预告不以空骨架冒充稳定结果；同一连续 progression 分为多个音频
checkpoint 时，只有出现可见里程碑才切换 state，否则复用。它们可以保留多个 Beat，
但不能仅凭 Beat 边界制造视觉变化。
...
- [ ] outline 让 chapter agent 知道本章固定什么；有变化时写清真实变化，无变化时
      明确复用同一 state，不为每个 Beat 强造视觉结果
- [ ] 默认使用一个持续 base-scene；accent 有真实强调目的
- [ ] custom-scene 为 `none` 或带必要性说明的 `proposed`，没有伪称已获批准
- [ ] 没有从槽位、recipe、时长或固定模板反推 Beat 数
- [ ] 若本章 Beat 数恰好等于 unique state 数，已重新检查同一 U/槽位的点名→解释、
      薄设问/预告和连续 progression 拆分；这是复核信号，不是自动失败或固定比例门槛
