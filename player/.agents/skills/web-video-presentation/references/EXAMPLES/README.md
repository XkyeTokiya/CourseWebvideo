...
| [`list-reveal/`](list-reveal/) | **真实顺序型** —— 阶段、步骤或排名拥有独立 narration beat | `chapter.tsx` + `chapter.css` |
| [`equal-group/`](equal-group/) | **并列等权型** —— 多个 step 可复用同一 semantic state，不轮流选中卡片 | README + `chapter.tsx` + `chapter.css` |
...
1. 先判断语义关系，再看哪个 anchor 跟这一章**关系最像**（钩子、真实顺序、并列等权或其它）
