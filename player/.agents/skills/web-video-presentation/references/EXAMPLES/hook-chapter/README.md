...
这里每张图已经拥有独立 narration beat，而且每个 beat 都要求观众单独读图，
所以示例为每张图保留一个 step。Courseplay 中，每个 step 应映射到章节内命名的
semantic state；这要求逐项聚焦，不要求为每项重新构图，仍可复用同一场景与
槽位。若批准口播没有这些独立 beat，不得仅根据图片数量反推更多 step。
