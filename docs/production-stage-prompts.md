...
1. 写入已确认的主题和开发模式。
2. 默认记录 `handoff=skipped` 并直接使用当前正式输入；只有明确请求、已证实上下文超预算、接收方不能安全读取正式输入的跨 Agent/任务/工作树交接，或诊断/兼容测试时，才为当前 A-page 生成 compact handoff，并记录 reason、consumer、lifecycle。handoff 不得用于补齐任务包、validation report 或其他非正式输入。
